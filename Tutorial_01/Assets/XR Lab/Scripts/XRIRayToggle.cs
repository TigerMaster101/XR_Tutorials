...
namespace XRLab
{
    ...
    [RequireComponent(typeof(UnityEngine.XR.Interaction.Toolkit.Interactors.XRRayInteractor))] //this will add the component to the attached game object if it is not already there
    public class XRIRayToggle : MonoBehaviour
    {
        ...
        [SerializeField] private UnityEngine.XR.Interaction.Toolkit.Interactors.XRRayInteractor m_ray; //a reference to the ray interactor used on the hand
        ...
        private void Awake()
        {
            m_ray = gameObject.GetComponent<UnityEngine.XR.Interaction.Toolkit.Interactors.XRRayInteractor>(); //getting the ray interactor. note if one was not added then one will be added automatically which could cause bugs if not setup correctly
